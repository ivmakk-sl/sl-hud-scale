...
- **Pure logic is separated and tested.** Logic that does not need the running game (which web view message starts the page script, and the call it gets) lives in `src/HudScaleLogic.cs` with no BepInEx or Il2Cpp reference, unit-tested under `tests/`. Flag new pure logic in `Plugin.cs`, and new pure logic with no test.
- **Patches** prefer a postfix, and tie the `Harmony` instance to the plugin GUID. The only patch is a Postfix on `WebUILayer.OnMessageFromJS`. It must not change the game's handling of the message.
...
- **No change at the game's values.** At `HudZoom = 1.3` and `HudTextScale = 1.0` (the game's own values, not the config defaults) the mod runs no script. Flag code that touches a page at these values.
- **Page script.** `src/page.js` runs in the root page and reaches the HUD pages (`CoreUI1`, `CoreUI0`) through their iframes. It scales each px font size from its original value (a second run gives the same result), writes the body zoom only when the value differs, and changes no page that is not a HUD page. `tests/page` runs it against the game's own `CoreUI1.html` and `CoreUI0.html`.
...
- **Config docs match the code.** A change to a `Config.Bind` default, range, or name also changes `CONFIG.md`.
