...
Nexus page images, tracked in the repo. The background is 1300x372, the gallery images are 1920x1080. The mods-grid thumbnail is not a separate file: Nexus picks it from the uploaded gallery images. The images were prepared by hand from in-game screenshots of one scene, at the game's own values and with the mod settings shown on each image. "Default" on the images means the game's own values (`HudZoom: 1.3`, `HudTextScale: 1`), not the mod's config defaults.

- `sl-hud-scale-banner.jpg` - the mod page background image (1300x372): the HUD panel of the settings window, with the Scale and Text Scale sliders.
